...
export async function POST(request: Request) {
    try {
        ...
        if (needsStyleChange) {
            ...
            if (refImage) {
                stylePrompt = `
**重要：添付した${modelImage.name}の人物に、${refImage.name}の髪型を移植してください。**

【品質】
添付した${modelImage.name}のモデル画像の”髪型以外”（顔、表情、体型、背景）を忠実に維持してください。

【目的】
ヘアスタイルを変更して、美容室に行く前に確認したいです。

【ヘアスタイル】
添付画像${refImage.name}の”ヘアスタイル”を**完全に再現**してください。
以下の要素を特に忠実にコピーしてください：
1. 髪の長さ（完全に同じ長さに）
2. 髪の質感（ストレート、ウェーブ、カールなど）
3. 前髪の形と分け目
4. 全体のシルエット

他の装飾（リボン、帽子など）やアクセサリーは参照しないでください。
                `;
            } else {
                stylePrompt = `
**重要：添付した${modelImage.name}の人物の髪型を、「${hairStyle}」に変更してください。**
...
【ヘアスタイル】
「${hairStyle}」の髪型を**忠実に再現**してください。
顔の形や雰囲気に合わせず、指定された髪型の特徴（長さ、質感、形状）を優先してください。
                `;
            }
        ...
        if (needsColorChange) {
            ...
            if (colorRefImage) {
                colorPrompt = `
**重要：添付した${modelImage.name}の人物に、${colorRefImage.name}の髪色を移植してください。**
...
【ヘアカラー】
添付画像${colorRefImage.name}の”ヘアカラー”を**完全に再現**してください。
以下の要素を特に忠実にコピーしてください：
1. 色味（Hue）
2. 明るさ（Brightness）
3. 彩度（Saturation）
...
他の装飾（リボン、帽子など）やアクセサリーは参照しないでください。
                ...
                colorPrompt = `
**重要：添付した${modelImage.name}の人物の髪色を、「${hairColor}」に変更してください。**
...
【ヘアカラー】
「${hairColor}」の髪色を**忠実に再現**してください。
元の髪色に影響されず、指定された色味を強く反映させてください。
                `;
            }
